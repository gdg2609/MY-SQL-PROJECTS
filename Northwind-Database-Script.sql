...
select*from orders where customerid=90;
...
#List all orders along with the customer name and employee name:
 select o.orderID,e.FirstName,e.LastName,c.CustomerName
 from orders o join employees e on o.EmployeeID=e.EmployeeID
 join customers c on o.CustomerID=c.customerID;

#Find out the number of products supplied by each supplier:
SELECT s.SupplierID, s.SupplierName, COUNT(p.ProductID) AS NumberOfProductsSupplied
FROM Suppliers s
...
select*from employees where birthdate between '1928-09-19'and '1955-03-04' ;


#Retrieve the details of a particular order along with the products ordered:
select* from orderdetails;
select od.orderdetailid, od.OrderID, od.productID, od.Quantity, p.productID, p.ProductName
...
select od.orderid,count(p.productid) as totalproducts
from orderdetails od join products p 
on od.ProductID=p.ProductID
group by od.orderid ;

#Retrieve the contact information of suppliers located in a specific country:
select contactname,SupplierName from suppliers where country="uk";

#Calculate the total revenue generated by each product:
select p.productid,p.ProductName,sum(p.price*od.quantity) as revenue
from orderdetails od join products p on p.productid=od.productid
group by p.productid,p.ProductName;

#Find the employee who handled the most orders:
select e.employeeid,e.firstname,count(o.orderid) as mostorder
...
select o.customerid,o.orderid,c.customername,s.shipperid,s.shippername from orders o
join shippers s on o.shipperid=s.shipperid
join customers c on o.customerid=c.customerid
where country="uk";
#Top Selling Products:
select p.productname,count(od.orderid) as topsellingproduct from orderdetails od
join products p on od.productid=p.ProductID
group by p.productname order by topsellingproduct desc;
...
join products p on od.ProductID=p.ProductID
group by c.CustomerName order by sales desc;


#supplier performance
select s.suppliername,count(p.productid) as totalproducts,sum(p.price)
from suppliers s join products p on p.SupplierID=s.SupplierID
group by s.SupplierName order by totalproducts desc;



 #Customer Geographic Analysis:

select country ,count(customerid)as totalsales from customers
